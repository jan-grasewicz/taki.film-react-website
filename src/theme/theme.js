...
export const theme = {
  ...
  // mainPadding: mainPaddings.map()
}

const checkWidth = media => {
  switch (media.matches) {
    case true:
      return console.log('bigger', media.media)
    case false:
      return console.log('smaller', media.media)
    default:
      return console.log('ups', media.media)
  }
}

{
  matchMedia &&
    Object.values(screens).map(screenSize => {
      const query = window.matchMedia(`(min-width: ${screenSize}px`)
      return query.addListener(checkWidth), checkWidth(query)
    })
}
